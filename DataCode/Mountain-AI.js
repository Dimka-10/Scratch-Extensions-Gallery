...
class MountainAI {
  ...
  async train(text, epochs=10, onEpoch=null) {
    ...
    const clipGrad = 1.0;  // Градиентный клиппинг
    const initialLR = this.learningRate;

    for(let epoch=1; epoch<=epochs; epoch++){
      if(onEpoch) onEpoch(epoch);

      // Уменьшаем LR с эпохами (пример: экспоненциальное снижение)
      this.learningRate = initialLR * Math.pow(0.95, epoch-1);
      ...
      for(let i=0; i < tokens.length - this.seqLen; i++){
        ...
        // Клиппинг градиентов по dL_dlogits
        for(let v=0; v<dL_dlogits.length; v++){
          if(dL_dlogits[v] > clipGrad) dL_dlogits[v] = clipGrad;
          else if(dL_dlogits[v] < -clipGrad) dL_dlogits[v] = -clipGrad;
        }

        // Обновление finalOutputWeights
        for(let d=0; d<this.embedDim; d++){
          const embVal = this.W_embed[inputSeq[this.seqLen - 1]][d];
          for(let v=0; v<this.vocab.length; v++){
            this.finalOutputWeights[d][v] -= this.learningRate * dL_dlogits[v] * embVal;
          }
        }

        // Обновление эмбеддингов (взвешено по градиенту для target токена)
        for(let d=0; d<this.embedDim; d++){
...
class MountainAIExtension {
  ...
  getInfo() {
    return {
      ...
      blocks: [
        ...
        {
          opcode: 'getCurrentLoss',
          blockType: Scratch.BlockType.REPORTER,
          text: Scratch.translate('current loss'),
        }
      ],
    };
  }
  ...
  getCurrentLoss() {
    return this.model.getCurrentLoss();
  }
}
